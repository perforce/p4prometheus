...
// It runs a tailer on p4p text log and feeds the lines to logparser, and
...
import (
	...
	"github.com/perforce/p4prometheus/cmd/p4plogtail/config"
...
// P4PLogTail structure
type P4PLogTail struct {
	config *config.Config
	logger *logrus.Logger
}

func newP4PLogTail(config *config.Config, logger *logrus.Logger) (p4l *P4PLogTail) {
	return &P4PLogTail{
		config: config,
		logger: logger,
	}
...
func (p4l *P4PLogTail) getTailer(cfgInput *logConfig) (fswatcher.FileTailer, error) {
...
func (p4l *P4PLogTail) runLogTailer(logger *logrus.Logger) {

	logcfg := &logConfig{
		Type:                 "file",
		Path:                 p4l.config.P4PLog,
			...
			case err := <-tailer.Errors():
				if err != nil {
					if os.IsNotExist(err.Cause()) {
						p4l.logger.Errorf("error reading p4p log lines: %v: use 'fail_on_missing_logfile: false' in the input configuration if you want p4plogtail to start even though the logfile is missing", err)
						os.Exit(-3)
					}
...
func main() {
	var (
		configfile = kingpin.Flag(
			"config",
			"Config file for p4plogtail.",
		).Short('c').Default("p4plogtail.yaml").String()
		p4plog = kingpin.Flag(
			"p4plog",
			"P4PLOG file to process (overrides value in config file if specified)",
		).Default("").String()
	...
	kingpin.Version(version.Print("p4plogtail"))
	...
	logger.Infof("%v", version.Print("p4plogtail"))
	logger.Infof("Config: %+v", *cfg)
	logger.Infof("Processing:  '%s', output to '%s'", cfg.P4PLog, cfg.JSONLog)

	if *p4plog != "" {
		if cfg.P4PLog != "" {
			logger.Warnf("--p4plog %q overwriting config value %q", *p4plog, cfg.P4PLog)
		}
		cfg.P4PLog = *p4plog
	}
	if *jsonlog != "" {
