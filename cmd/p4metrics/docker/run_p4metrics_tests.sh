...
# Assume new version of p4metrics is in /tmp
cd /tmp
...
if [[ $UseSDP -eq 1 ]]; then
    cd /p4/common/config
    cp p4metrics.yaml p4metrics.yaml.bak
    sed -e 's/update_interval: .*/update_interval: 10s/' p4metrics.yaml.bak > p4metrics.yaml
    chown perforce: p4metrics.yaml
    cd /root
    py.test -v test_p4metrics.py
else
